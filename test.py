import string, discord, random, time, typing
from typing import List
from discord import Intents
# import TicTacToeMaster as ttt uncomment later
...
async def on_message(message):
    ...
    message_sent: str = str(message.content)
    messages: List[str] = message_sent.split()
    ...
    for i in range(len(messages) - 1):
        word: str = messages[i].lower()
        next_word: str = messages[i+1].lower()
        add_up_return: str = ''
...
"""def range_gen(iterator, start=0, step=1, end=None):
    if end is None:
        end = len(iterator)
    i = start
    while i < end:
        yield (i, iterator[item])
        i += 1"""
...
def get_quotes() -> List[str]: 
    ...
    a = ['\"I\'m hungry, I\'m gonna go eat my cookie\"', '\"Yo I didn\'t know you guys kept gatorade in here!\" * grabs windex *', '\"I don\'t have insomnia I just can\'t sleep at night\"']
